...
class StubDatabricksService:
    ...
    async def get_sql_warehouse_cost_summary(self, warehouse_id):
...
def make_llm(query_impl):
    ...
    llm.client = SimpleNamespace(
        serving_endpoints=SimpleNamespace(query=query_impl)
    )
    return llm
...
check('analysis is LLM text (stripped)',
...
check('SYSTEM prompt is SQL_WAREHOUSE_ANALYSIS_PROMPT',
      system_msg.content == SQL_WAREHOUSE_ANALYSIS_PROMPT)
check(f'max_tokens == {SQL_WAREHOUSE_MAX_TOKENS}',
      captured['max_tokens'] == SQL_WAREHOUSE_MAX_TOKENS)

um = user_msg.content
check('USER msg carries warehouse type', 'Warehouse Type: PRO' in um)
check('USER msg flags auto-stop > 30 threshold',
...
check('USER msg states DBU is the complete cost',
      'DBU is the COMPLETE cost' in um)
check('USER msg has NO cloud-VM caveat from other tabs',
      'excludes cloud VM cost' not in um and 'idle-vs-active' not in um)
...
for section in ('## 1. Overall Rating', '## 2. Right-Sizing Assessment',
                '## 3. Cost Optimization', '## 4. Configuration Gaps',
                '## 5. Recommendations'):
...
check('fallback leaks no raw exception',
      'serving endpoint unavailable' not in analysis and 'Traceback' not in analysis)
check('fallback has NO cloud-cost caveat',
      'excludes cloud VM cost' not in analysis and 'idle-vs-active' not in analysis)
...
check('cost summary rendered as unavailable',
      'Cost data unavailable.' in seen.get('user', ''))
...
check('generic detail, no internals leaked',
      'details query failed' not in resp.text)
...
zero = {'total_cost': 0.0, 'total_dbu_cost': 0.0, 'active_days': 0,
        'avg_daily_cost': 0.0, 'warehouse_type': None, 'warehouse_name': None,
        'lookback_days': 30}
client = build_client(
    StubDatabricksService(details=missing, cost=zero), make_llm(capture_query)
)
...
check('zero-spend window stated',
      'No spend data available for this warehouse' in um)
