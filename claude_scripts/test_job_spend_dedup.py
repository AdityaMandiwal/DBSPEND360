...
def _exec(client: WorkspaceClient, warehouse_id: str, sql: str):
    ...
    if not resp.status or resp.status.state != StatementState.SUCCEEDED:
        ...
        raise RuntimeError(f"Query failed ({resp.status.state if resp.status else 'no status'}): {err}\nSQL:\n{sql}")
    return resp
...
def _assert_top_jobs_aggregation(
    ...
    rows = _run_top_jobs_query(
        client, warehouse_id, table_name, start_date, end_date, limit=limit
    )
    print(f"  rows returned: {len(rows)}")
    for row in rows:
        # row = [job_id, total_cloud_cost, total_databricks_cost, run_count, name, total_cost]
        print(f"    job_id={row[0]!s:>20}  total_cost={float(row[5]):>12.4f}  runs={row[3]}  name={row[4]}")
    ...
    print(
        f"PASS: top-jobs #1 == grouped-jobs #1 "
        f"(job_id={top_job_id}, total_cost={top_total:.4f})."
    )


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--job-id", default=KNOWN_RENAMED_JOB_ID,
    ...
    parser.add_argument("--table", default=None,
                        help="Fully-qualified spend table name (defaults to app config)")
    parser.add_argument("--days", type=int, default=DEFAULT_LOOKBACK_DAYS,
                        help=f"Lookback window in days (default: {DEFAULT_LOOKBACK_DAYS})")
    parser.add_argument("--compare-buggy", action="store_true",
    ...
    if total_rows == 0:
        print("  WARNING: job not present in system.lakeflow.jobs — bug cannot be reproduced for this id.")
    elif distinct_names < 2:
        print("  NOTE: job has only one distinct name in SCD history — this id no longer exercises the bug.")
    print()
    ...
    if len(rows) == 0:
        print("WARNING: no spend rows in the selected window for this job. Cannot assert dedup behaviour.")
    ...
    print("-" * 60)
    _assert_top_jobs_aggregation(
        client, warehouse_id, table_name, start_date, end_date, limit=5
    )
    return 0
