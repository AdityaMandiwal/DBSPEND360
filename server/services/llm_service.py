...
# Mandatory honesty string when classic/mixed pipeline cloud coverage is
# incomplete. The prompt and structured fallback both preserve this disclosure.
PIPELINE_CLOUD_COVERAGE_CAVEAT = (
    'Total Spend excludes cloud VM cost where cloud coverage is unavailable'
)

# Mandatory honesty string for instance-pool analysis. The pool cloud explorer
# deliberately nets out rows that also carry ClusterId, so this tab contains
# only ClusterId-free idle/warm capacity. Active pool-backed VM cost remains on
# the Job or All-Purpose lens. The prompt mandates this phrase and the fallback
# embeds it too so the model never describes the pool cloud line as full VM cost.
POOL_CLOUD_SCOPE_CAVEAT = 'active pool-backed VM cost is attributed to the Job or All-Purpose tab and is not included here'
...
5. If no optimization exists, state: "No actionable optimizations identified" and briefly explain why.
6. NEVER fabricate cost estimates or reference external benchmarks.
7. **Cloud-cost scope (MANDATORY).** The cloud line contains only
   ClusterId-free idle/warm pool capacity. Active pool-backed VMs carry
   ClusterId and are netted out of this tab; their cloud cost stays on the Job
   or All-Purpose tab. You MUST include the exact phrase "active pool-backed VM
   cost is attributed to the Job or All-Purpose tab and is not included here"
   at least once. Treat the cloud line as warm-capacity cost, not full pool VM
   cost, and do not claim every warm dollar is avoidable waste.
...
9. Configuration Gaps (besides the mandatory cloud-scope caveat): ONLY list
...
ratio of distinct attached clusters to cost-bearing days:
...
- `min_idle_instances` vs `peak_concurrent_clusters` — if min idle persistently
  ...
  not being claimed. Quantify the over-provisioning in instances and reference
  the idle/warm cloud cost, but do not assume all warm-capacity cost is
  avoidable.
- `idle_instance_autotermination_minutes` — long values keep VMs warm
  ...
  cluster-fanout. NEVER claim that increasing this timeout reduces cost:
  longer retention can only keep excess instances alive longer. Recommend an
  increase only for documented latency/churn needs, not as a savings action.
...
- Ratio `distinct_cluster_count / active_days` — `active_days` means days with
  a pool cost row, including idle-only days. High fanout (>10/day on
...
- Each MUST reference >= 1 specific configuration or cost metric from input.
- Dollar impact MUST be derived from input Cost Summary numbers. Use the
  idle/warm cloud line for warm-capacity discussion; do not describe it as full
  pool VM cost or fabricate avoidable-savings amounts. If impact cannot be
  quantified, say so.
...
- Assess using `min_idle_instances`, `idle_instance_autotermination_minutes`,
  ...
  with the mandatory reminder that active pool-backed VM cost is attributed to
  the Job or All-Purpose tab and is not included here.
...
5. If no optimization exists, state: "No actionable optimizations identified" and briefly explain why.
6. NEVER fabricate cost estimates or reference external benchmarks.
7. **Cloud-coverage honesty (MANDATORY, conditional).** Use `Cloud Coverage
   Complete`, not `Cost Basis`, to decide whether spend is complete:
   - `Yes`: Total Spend includes all available Databricks DBU and cloud cost.
     Do not add a missing-cloud caveat.
   - `No`: You MUST include the exact phrase "excludes cloud VM cost" at least
     once and must not invent savings for the unavailable cloud portion.
   - Serverless has no separate cloud VM line. Do not recommend cloud-VM or
     instance-type changes for serverless compute.
8. NEVER cite industry-average savings percentages or invent annualized dollar
   ranges not derived from the input Cost Summary.
...
10. Configuration Gaps: ONLY cost-driving or mandatory cloud-coverage caveats.
...
- `Compute Mode` (serverless / classic / mixed) — serverless has no separate
  cloud VM line; classic/mixed cloud completeness is stated explicitly.
...
- Dollar impact MUST be derived from input numbers, or state impact is not
  quantifiable. When cloud coverage is incomplete, do not estimate missing
  cloud cost or savings.
...
- For classic / mixed workloads: assess idle compute using the reported cloud
  line; add the missing-cloud disclosure only when coverage is incomplete.

## Section 5 — Configuration Gaps

- Omit filler; if no cost-driving gaps (and no mandatory cloud-coverage caveat),
  write exactly: "None — no cost-driving configuration gaps identified."
...
Use the `Cost Basis` field:

- `full` (Serverless): DBU includes infrastructure. Do not add a missing-cloud
  caveat or recommend VM-level changes.
- `dbu_only` (Classic/Pro): reported spend excludes customer-cloud VM, disk,
  and network charges. You MUST state this once and must not estimate the
  missing amount. Optimize observed DBU using warehouse size, cluster scaling,
  auto-stop, and query efficiency.
- `unknown`: state that cost completeness cannot be determined.
...
- State cost completeness from `Cost Basis`. Never call Classic/Pro DBU a
  complete total.
...
class LLMService:
    ...
    async def analyze_cluster_configuration(
        ...
        try:
            user_message = self._build_cluster_user_message(
                cluster_details, cost_summary, cluster_kind
            )
    ...
    async def analyze_instance_pool_costs(
        ...
        ratio). The cost summary contains ClusterId-free idle/warm cloud cost;
        the prompt mandates ``POOL_CLOUD_SCOPE_CAVEAT`` so the model does not
        mistake it for complete active + idle VM cost.
        ...
        """
        try:
            user_message = self._build_pool_user_message(pool_details, cost_summary)
    ...
    async def analyze_pipeline_costs(
        ...
        ``Cloud Coverage Complete`` is the correctness-critical input: the
        prompt adds the ``excludes cloud VM cost`` disclosure only when cloud
        coverage is incomplete. Serverless retains the no-separate-VM guidance.
        The structured fallback honours the same conditional on LLM failure.
        ...
        """
        try:
            user_message = self._build_pipeline_user_message(pipeline_details, cost_summary)
    ...
    async def analyze_sql_warehouse_costs(
        ...
        Cost completeness is type-dependent: Serverless DBU includes
        infrastructure, while Classic/Pro tracked spend excludes customer-cloud
        infrastructure. The warehouse-specific optimization angle includes
        auto-stop tuning (``auto_stop_mins`` above
        ...
        """
        try:
            user_message = self._build_sql_warehouse_user_message(warehouse_details, cost_summary)
            ...
            return self._build_sql_warehouse_fallback(warehouse_details, cost_summary)
    ...
    def _build_cluster_user_message(
        ...
        if cluster.min_autoscale_workers is not None:
            autoscale = f'Autoscaling: {cluster.min_autoscale_workers}–{cluster.max_autoscale_workers} workers'
        ...
        provider_label, provider_availability, spot_bid_pct = self._extract_provider_attributes(
            cluster
        )
        ...
        if cost_summary is not None and cost_summary.get('total_run_count', 0) > 0:
            ...
            if cluster_kind == 'all_purpose':
                lines.append(
                    f'- Owners: {cost_summary.get("distinct_user_count", 0)} distinct | '
                    f'Active cluster-days: {cost_summary["total_run_count"]}'
                )
                lines.append(f'- Avg Cost/Cluster-Day: ${cost_summary["avg_cost_per_run"]:,.2f}')
            else:
                lines.append(
    ...
    def _build_pool_user_message(
        ...
        if pool.pool_creator_id:
            creator_line = (
                f'- Creator ID: {pool.pool_creator_id} (GUID only — v1 does not resolve to email)'
            )
        else:
        ...
        lines: list[str] = [
            ...
            (
                f'- Idle Autotermination: {_fmt_int(pool.idle_instance_autotermination_minutes)} minutes'
            ),
        ...
        if cost_summary is not None and cost_summary.get('active_days', 0) > 0:
            ...
            lines.append(
                f'- Total Spend (DBU + idle/warm cloud): ${cost_summary["total_spend"]:,.2f}'
            )
            lines.append(f'- Databricks Cost (DBU): ${cost_summary["total_databricks_cost"]:,.2f}')
            pool_cloud = cost_summary.get('total_cloud_cost')
            if pool_cloud is not None:
                lines.append(f'- Idle/Warm Cloud VM Cost (ClusterId-free): ${pool_cloud:,.2f}')
            else:
                lines.append(
                    '- Idle/Warm Cloud VM Cost: not available for this window '
                    '(no pool-tag cloud row landed yet)'
                )
            lines.append(f'- Distinct Clusters Attached: {cost_summary["distinct_cluster_count"]}')
            lines.append(f'- Cost Days (including idle-only): {cost_summary["active_days"]}')
            lines.append(
                f'- Peak Concurrent Clusters (max distinct on any single day): '
                f'{cost_summary["peak_concurrent_clusters"]}'
            )
            ratio = (
                cost_summary['distinct_cluster_count'] / cost_summary['active_days']
                if cost_summary['active_days'] > 0
                else 0.0
            )
            lines.append(f'- Cluster Fanout (distinct_clusters / active_days): {ratio:.2f}')
            lines.append(f'- Avg Cost / Pool-Day: ${cost_summary["avg_cost_per_pool_day"]:,.2f}')
            first = cost_summary.get('first_active_date') or 'N/A'
            last = cost_summary.get('last_active_date') or 'N/A'
            lines.append(f'- Cost Period: {first} to {last}')
            ...
            if cost_summary.get('limited_history'):
                lines.append(
                    '- NOTE: limited history (<3 active days) — trend signals are unreliable.'
                )
        elif cost_summary is not None:
            lines.append(f'## Cost Summary ({LOOKBACK_DAYS}-day window)')
            lines.append('No spend data available for this pool in the lookback window.')
        else:
            lines.append('## Cost Summary')
            lines.append('Cost data unavailable.')
        ...
        lines.extend(
            [
                '',
                '## Notes',
                '- Cloud-cost scope: the cloud line contains only ClusterId-free '
                'idle/warm pool capacity. '
                f'{POOL_CLOUD_SCOPE_CAVEAT}.',
            ]
        )
    ...
    def _pipeline_effective_cost_basis(
        ...
        """Resolve the cost basis shown as pipeline context.

        Prefer the window-scoped value from the cost summary and fall back to
        the pipeline snapshot's dimension. Cloud coverage, not this label,
        controls the missing-cloud disclosure.
        """
    ...
    @staticmethod
    def _pipeline_caveat_applies(
        cost_summary: Optional[dict],
        compute_mode: Optional[str],
    ) -> bool:
        """Return whether missing classic/mixed cloud cost must be disclosed."""
        if cost_summary and 'cloud_coverage_complete' in cost_summary:
            return not bool(cost_summary['cloud_coverage_complete'])
        return compute_mode in ('classic', 'mixed')
    ...
    def _build_pipeline_user_message(
        ...
        Renders workload/compute context, metadata state, owner attribution,
        and cost shape. Cloud coverage, rather than ``cost_basis``, controls
        the missing-cloud disclosure.
        """
        cost_basis = self._pipeline_effective_cost_basis(pipeline, cost_summary)
        workload_type = (
            (cost_summary or {}).get('workload_type') or pipeline.workload_type or 'Unknown'
        )
        compute_mode = (
            (cost_summary or {}).get('compute_mode') or pipeline.compute_mode or 'Unknown'
        )
        caveat_applies = self._pipeline_caveat_applies(cost_summary, compute_mode)
        ...
        if cost_summary is not None and cost_summary.get('active_days', 0) > 0:
            ...
            lines.append(f'## Cost Summary ({lookback}-day window)')
            lines.append(f'- Total Spend: ${cost_summary["total_spend"]:,.2f}')
            lines.append(f'- Databricks DBU Cost: ${cost_summary["total_databricks_cost"]:,.2f}')
            cloud_cost = cost_summary.get('total_cloud_cost')
            lines.append(
                '- Cloud Cost: '
                + (
                    f'${cloud_cost:,.2f}'
                    if isinstance(cloud_cost, (int, float))
                    else 'Not available'
                )
            )
            lines.append(
                '- Cloud Coverage Complete: '
                + ('Yes' if cost_summary.get('cloud_coverage_complete') else 'No')
            )
            lines.append(f'- Active Days: {cost_summary["active_days"]}')
            lines.append(f'- Avg Cost / Active Day: ${cost_summary["avg_cost_per_day"]:,.2f}')
            lines.append(
                f'- Distinct Workload Types: {cost_summary.get("distinct_workload_count", 1)}'
            )
            first = cost_summary.get('first_active_date') or 'N/A'
            last = cost_summary.get('last_active_date') or 'N/A'
            lines.append(f'- Active Period: {first} to {last}')
            ...
            lines.append(f'## Cost Summary ({LOOKBACK_DAYS}-day window)')
            lines.append('No spend data available for this pipeline in the lookback window.')
        else:
            lines.append('## Cost Summary')
            lines.append('Cost data unavailable.')
        ...
        if caveat_applies:
            lines.append(
                f'- Coverage caveat: {PIPELINE_CLOUD_COVERAGE_CAVEAT}. '
                'Do not invent cloud-VM cost or savings for missing coverage.'
            )
        elif compute_mode == 'serverless':
            lines.append(
                '- Serverless has no separate cloud VM line; do not recommend '
                'cloud-VM or node-type changes.'
            )
        else:
            lines.append(
                '- Cloud coverage is complete; Total Spend includes the '
                'reported Databricks DBU and cloud costs.'
            )
    ...
    def _build_sql_warehouse_user_message(
        ...
        and the cost shape over the requested window. The `Notes` section
        explicitly distinguishes Serverless complete cost from Classic/Pro
        DBU-only spend.
        """
        warehouse_type = (
            (cost_summary or {}).get('warehouse_type') or warehouse.warehouse_type or 'Unknown'
        )
        cost_basis = (cost_summary or {}).get('cost_basis') or warehouse.cost_basis or 'unknown'
        if cost_basis == 'full':
            cost_scope_note = (
                '- Cost scope: Serverless DBU includes infrastructure; no '
                'separate cloud VM line is missing.'
            )
        elif cost_basis == 'dbu_only':
            cost_scope_note = (
                '- Cost scope: Classic/Pro tracked spend is DBU-only and '
                'excludes customer-cloud VM, disk, and network charges. Do not '
                'estimate the missing amount.'
            )
        else:
            cost_scope_note = (
                '- Cost scope: completeness is unknown because warehouse type is unavailable.'
            )
        ...
        if cost_summary is not None and cost_summary.get('active_days', 0) > 0:
            lookback = cost_summary.get('lookback_days', LOOKBACK_DAYS)
            start_date = cost_summary.get('start_date')
            end_date = cost_summary.get('end_date')
            window_label = (
                f'{start_date} to {end_date}'
                if start_date and end_date
                else f'{lookback}-day window'
            )
            lines.append(f'## Cost Summary ({window_label})')
            lines.append(f'- Tracked DBU Spend: ${cost_summary["total_cost"]:,.2f}')
            lines.append(f'- Databricks Cost (DBU): ${cost_summary["total_dbu_cost"]:,.2f}')
            lines.append(f'- Active Days: {cost_summary["active_days"]}')
            lines.append(f'- Avg Cost / Active Day: ${cost_summary["avg_daily_cost"]:,.2f}')
            ...
            lines.append('## Cost Summary')
            lines.append('No spend data available for this warehouse in the lookback window.')
        else:
            lines.append('## Cost Summary')
            lines.append('Cost data unavailable.')
    ...
    def _build_cluster_fallback(
        ...
        if cluster_details.is_job_cluster:
            lines.append(
                '- Not applicable \u2014 ephemeral job cluster terminates on run completion.'
            )
        else:
    ...
    def _build_pool_fallback(
        ...
        The mandatory ``POOL_CLOUD_SCOPE_CAVEAT`` appears in the fallback so
        scope remains accurate even when the LLM call itself fails.
        """
        pool_name = pool_details.pool_name or f'Pool {pool_details.instance_pool_id}'
        node_type = pool_details.node_type or 'N/A'
        min_idle = (
            str(pool_details.min_idle_instances)
            if pool_details.min_idle_instances is not None
            else 'N/A'
        )
        max_cap = str(pool_details.max_capacity) if pool_details.max_capacity is not None else 'N/A'
        autoterm = (
        ...
        if (
            ...
            lines.append(f'- Total Spend (DBU + EC2/EBS): ${cost_summary["total_spend"]:,.2f}')
            pool_cloud = cost_summary.get('total_cloud_cost')
            if pool_cloud is not None:
                lines.append(f'- Cloud VM Cost (EC2/EBS): ${pool_cloud:,.2f}')
            lines.append(
                f'- Distinct Clusters: {cost_summary.get("distinct_cluster_count", "N/A")}'
            )
            lines.append(f'- Active Days: {cost_summary.get("active_days", "N/A")}')
            lines.append(
                f'- Peak Concurrent Clusters: {cost_summary.get("peak_concurrent_clusters", "N/A")}'
            )
        else:
            lines.append('- No cost data available for sizing assessment')
        lines.extend(
            [
                ...
                '## 4. Idle Waste Risk',
                f'- Idle Autotermination: {autoterm}',
                '- Detailed analysis unavailable. The cloud line contains only '
                'ClusterId-free idle/warm capacity.',
                '',
                '## 5. Configuration Gaps',
                '- Automated analysis could not be generated',
                f'- Reminder: {POOL_CLOUD_SCOPE_CAVEAT}.',
            ]
        )
        return '\n'.join(lines)
    ...
    def _build_pipeline_fallback(
        ...
        Honours the prompt's cloud-coverage conditional, including the
        ``excludes cloud VM cost`` disclosure only for incomplete coverage.
        """
        cost_basis = LLMService._pipeline_effective_cost_basis(pipeline_details, cost_summary)
        pipeline_name = pipeline_details.pipeline_name or f'Pipeline {pipeline_details.pipeline_id}'
        workload_type = (
            (cost_summary or {}).get('workload_type') or pipeline_details.workload_type or 'Unknown'
        )
        compute_mode = (
            (cost_summary or {}).get('compute_mode') or pipeline_details.compute_mode or 'Unknown'
        )
        caveat_applies = LLMService._pipeline_caveat_applies(cost_summary, compute_mode)
        ...
        if (
            ...
            lines.append(f'- Total Spend: ${cost_summary["total_spend"]:,.2f}')
            lines.append(
                f'- Databricks DBU Cost: ${cost_summary.get("total_databricks_cost", 0):,.2f}'
            )
            cloud_cost = cost_summary.get('total_cloud_cost')
            lines.append(
                '- Cloud Cost: '
            ...
            lines.append(f'- Active Days: {cost_summary.get("active_days", "N/A")}')
            lines.append(f'- Avg Cost/Day: ${cost_summary.get("avg_cost_per_day", 0):,.2f}')
        else:
            lines.append('- No cost data available for sizing assessment')
        lines.extend(
            [
        ...
        if caveat_applies:
            lines.append(
                f'- Detailed analysis unavailable. Note: {PIPELINE_CLOUD_COVERAGE_CAVEAT}.'
            )
        elif compute_mode == 'serverless':
            lines.append(
            ...
            lines.append('- Detailed analysis unavailable. Cloud coverage is complete.')
        lines.extend(
            [
                '',
                '## 5. Configuration Gaps',
        ...
        if caveat_applies:
            lines.append(f'- Reminder: {PIPELINE_CLOUD_COVERAGE_CAVEAT}.')
        return '\n'.join(lines)
    ...
    def _build_sql_warehouse_fallback(
        ...
        shape on LLM failure and preserves the warehouse cost-basis disclosure.
        """
        warehouse_name = (
        ...
        cost_basis = (
            (cost_summary or {}).get('cost_basis') or warehouse_details.cost_basis or 'unknown'
        )
        ...
        min_clusters = (
            ...
            if warehouse_details.min_clusters is not None
            else 'N/A'
        )
        max_clusters = (
            str(warehouse_details.max_clusters)
            if warehouse_details.max_clusters is not None
            else 'N/A'
        )
        ...
        if (
            ...
            lines.append(f'- Tracked DBU Spend: ${cost_summary["total_cost"]:,.2f}')
            lines.append(f'- Active Days: {cost_summary.get("active_days", "N/A")}')
            lines.append(f'- Avg Cost/Day: ${cost_summary.get("avg_daily_cost", 0):,.2f}')
        else:
            lines.append('- No cost data available for sizing assessment')
        lines.extend(
            [
                ...
                '## 3. Cost Optimization',
                (
                    '- Detailed analysis unavailable. Serverless DBU includes infrastructure.'
                    if cost_basis == 'full'
                    else '- Detailed analysis unavailable. Classic/Pro tracked '
                    'spend is DBU-only and excludes customer-cloud infrastructure.'
                ),
                f'- Auto-Stop: {auto_stop}',
                '',
                '## 4. Configuration Gaps',
    ...
    def _format_cluster_type_line(cluster: ClusterDetails) -> str:
        ...
        if cluster.is_job_cluster:
            return 'Cluster Type: JOB cluster (ephemeral, lifecycle-bound to the run — auto-termination N/A)'
        if cluster.cluster_source:
            return f'Cluster Type: {cluster.cluster_source} cluster (interactive — auto-termination applies)'
