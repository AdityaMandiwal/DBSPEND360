...
def _parse_workspace_covered(value) -> bool:
  """Parse `workspace_covered` from a SQL row (bool or Spark string)."""
  if value is None:
    return True
...
class DatabricksService:
  ...
  def __init__(self):
    ...
    # Lazy cache: whether each rollup table has `workspace_covered` yet.
    self._workspace_covered_column_cache: Dict[str, bool] = {}

  def _statement_wait_timeout(self) -> str:
    """Configured synchronous wait for SQL warehouse statements."""
    return f'{self.query_timeout}s'

  def _execute_statement(self, statement: str, **kwargs):
    """Execute a warehouse statement and wait for completion.

    Expansion queries (per-user cluster breakdown, per-cluster day
    breakdown) join `system.compute.clusters` and are slower than the
    parent rollup. Without an explicit `wait_timeout`, the SDK's short
    default causes `PENDING` responses whose empty `data_array` was
    previously treated as "no rows".
    """
    ...
    response = self.client.statement_execution.execute_statement(
      ...
      statement=statement,
      wait_timeout=kwargs.pop('wait_timeout', self._statement_wait_timeout()),
      **kwargs,
    )
  ...
  def _table_has_workspace_covered(self, table_name: str) -> bool:
    """Return True when `workspace_covered` exists on the rollup table."""
    if table_name in self._workspace_covered_column_cache:
      return self._workspace_covered_column_cache[table_name]
    try:
      ...
      resp = self.client.statement_execution.execute_statement(
        ...
        statement=(
          ...
          f"AND column_name = 'workspace_covered' "
          f'LIMIT 1'
        ),
      )
  ...
  def _workspace_covered_sql(self, table_name: str, alias: str = '') -> str:
    """SQL expression for workspace coverage, safe before ETL adds the column."""
    prefix = f'{alias}.' if alias else ''
    if self._table_has_workspace_covered(table_name):
      return f'COALESCE({prefix}workspace_covered, true)'
    return 'true'

  def _workspace_covered_agg_sql(self, table_name: str, alias: str = '') -> str:
    """Aggregation helper for `workspace_covered` in GROUP BY queries."""
    prefix = f'{alias}.' if alias else ''
    if self._table_has_workspace_covered(table_name):
      return f'BOOL_AND(COALESCE({prefix}workspace_covered, true)) AS workspace_covered'
    return 'true AS workspace_covered'
  ...
  async def get_summary_metrics(self, start_date: date, end_date: date) -> SummaryMetrics:
    ...
    """
    wsc_agg = self._workspace_covered_agg_sql(self.table_name)
    ...
    query = f"""
        ...
        job_level AS (
            SELECT
                ...
                SUM(network_cost) AS total_network_cost,
                SUM(other_cost) AS total_other_cost,
                BOOL_AND(workspace_covered) AS workspace_covered,
                COUNT(*) AS run_count
            FROM run_level
            GROUP BY job_id
        )
  ...
  async def _get_batch_job_runs(
    ...
    query = f"""
        ...
        SELECT job_id, run_id, cluster_id, start_date, end_date,
               ...
               total_other_cost, workspace_covered
        FROM ranked_runs
    ...
    if response.result and response.result.data_array:
      for row in response.result.data_array:
        run = JobRun(
          ...
          workspace_covered=_parse_workspace_covered(row[10]),
        )
        runs.append(run)
  ...
  def _product_label(self, billing_origin_product: str) -> str:
    return _JOB_PRODUCT_LABELS.get(billing_origin_product, billing_origin_product)
  ...
  async def get_job_product_breakdown(
    ...
    if total_unpriced_qty > 0:
      unpriced_warning = 'Some usage rows had no matching list price; breakdown may be understated.'
  ...
  async def _detect_cluster_kind(self, cluster_id: str) -> Literal['job', 'all_purpose']:
  ...
  async def _get_cluster_cost_summary_job(self, cluster_id: str) -> Optional[dict]:
  ...
  async def _get_cluster_cost_summary_all_purpose(self, cluster_id: str) -> Optional[dict]:
  ...
  async def get_all_purpose_grouped_by_user(
    ...
    total_count = 0
    if data_response.result and data_response.result.data_array:
      total_count = int(data_response.result.data_array[0][10])
  ...
  async def get_pool_metadata(self, pool_id: str) -> Tuple[str, Optional[str]]:
  ...
  async def get_instance_pool_daily_trend(
    self, start_date: date, end_date: date
  ) -> List[InstancePoolDailyTrendPoint]:
    ...
    if response.result and response.result.data_array:
      for row in response.result.data_array:
        ...
        by_day[day] = float(row[1]) if row[1] is not None else 0.0

    points: List[InstancePoolDailyTrendPoint] = []
    cursor = start_date
    while cursor <= end_date:
      points.append(
        InstancePoolDailyTrendPoint(
          usage_date=cursor,
          total_cost=by_day.get(cursor, 0.0),
        )
      )
  ...
  async def get_instance_pools_grouped(
    ...
    if data_response.result and data_response.result.data_array:
      ...
      days_by_pool = await self._get_batch_pool_days_and_clusters(pool_ids, start_date, end_date)
    ...
    for pool_id, pool_days in days_by_pool.items():
      ...
      for day in ordered_days:
        ...
        day.cluster_count_on_day = sum(
          1 for c in day.clusters if c.cluster_id != '__pool_overhead__'
        )
  ...
  async def get_instance_pool_details(self, pool_id: str) -> InstancePoolDetails:
    ...
    try:
      ...
      logger.error('Error fetching pool snapshot for %s: %s', pool_id, str(exc))
    ...
    return InstancePoolDetails(
      ...
      idle_instance_autotermination_minutes=int(snapshot_row[5])
      if snapshot_row[5] is not None
      else None,
  ...
  async def _resolve_pipeline_workspace(
    ...
    * >1 candidates -> raises `AmbiguousPipelineError` (router → HTTP
    ...
    candidates: List[str] = []
    if response.result and response.result.data_array:
      candidates = [row[0] for row in response.result.data_array if row[0] is not None]
  ...
  async def get_pipelines_grouped(
    ...
    *labels/filters*, never drops). `days` (per-day expansion) is
    ...
    total_count = 0
    if data_response.result and data_response.result.data_array:
      total_count = int(data_response.result.data_array[0][16])

    grouped: List[GroupedPipeline] = []
    if data_response.result and data_response.result.data_array:
      id_pairs = [(row[0], row[1]) for row in data_response.result.data_array]
  ...
  async def _get_batch_pipeline_days(
    ...
    in_clause = ', '.join(f"'{pid.replace(chr(39), chr(39) * 2)}'" for pid in pipeline_ids)
    workload_filter = self._pipeline_workload_filter(workload_type)
      ...
      for row in response.result.data_array:
        ...
        result.setdefault(key, []).append(
          PipelineDailySpend(
            ...
            workspace_covered=_parse_workspace_covered(row[7]),
          )
        )
    ...
    if breakdown_response.result and breakdown_response.result.data_array:
      for row in breakdown_response.result.data_array:
        if row[0] is not None:
          workload_breakdown[row[0]] = float(row[1]) if row[1] is not None else 0.0
  ...
  async def get_pipeline_details(
    ...
    """
    resolved_workspace = await self._resolve_pipeline_workspace(pipeline_id, workspace_id)
    escaped_id = pipeline_id.replace("'", "''")
  ...
  async def get_coverage_summary(self) -> CoverageSummaryResponse:
    ...
    absent from `dbspend360_covered_workspaces`. Per-tab excluded DBU is
    ...
    subs_query = f"""
        ...
        FROM {self.covered_workspaces_table_name}
    ...
    excluded_query = f"""
        ...
        recent_usage AS (
            SELECT
                ...
                SUM(
                    u.usage_quantity
                    * CAST(lp.pricing['default'] AS DOUBLE)
                ) AS dbu_dollars
            FROM system.billing.usage u
            LEFT JOIN system.billing.list_prices lp
              ON u.sku_name = lp.sku_name
        ...
        SELECT e.workspace_id, w.workspace_name, ROUND(e.dbu_dollars, 2) AS dbu_dollars
        FROM recent_usage e
        LEFT JOIN covered c ON e.workspace_id = c.workspace_id
        LEFT JOIN system.access.workspaces_latest w ON e.workspace_id = w.workspace_id
        WHERE c.workspace_id IS NULL
        ORDER BY e.dbu_dollars DESC
        LIMIT 10
        """
    tab_query = f"""
        ...
        for row in excluded_resp.result.data_array:
          excluded_workspaces.append(
            ExcludedWorkspace(
              ...
              dbu_dollars=float(row[2]) if row[2] is not None else 0.0,
            )
          )
    except Exception as e:
    ...
    try:
      ...
      logger.warning(
        'Per-tab excluded DBU query failed (workspace_covered column may '
        'not exist until ETL runs): %s',
        e,
      )

    return CoverageSummaryResponse(
      covered_subscription_ids=covered_subscription_ids,
