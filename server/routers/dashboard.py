...
async def get_job_spends(
  ...
  try:
    ...
    result = await service.get_job_spends(
      start_date=start_date, end_date=end_date, job_name=job_name, limit=per_page, offset=offset
    )

    return result

  except Exception as e:
    raise HTTPException(status_code=500, detail=f'Error retrieving job spending data: {str(e)}')
...
async def get_grouped_job_spends(
  ...
  try:
    ...
    raise HTTPException(
      status_code=500, detail=f'Error retrieving grouped job spending data: {str(e)}'
    )
...
async def get_summary_metrics(
  ...
  try:
    ...
    service = get_databricks_service()
    result = await service.get_summary_metrics(start_date=start_date, end_date=end_date)

    return result

  except Exception as e:
    raise HTTPException(status_code=500, detail=f'Error retrieving summary metrics: {str(e)}')


@router.get('/job/{job_id}/breakdown', response_model=CostBreakdown)
async def get_job_cost_breakdown(
  job_id: str, run_id: str = Query(..., description='Run ID for the specific job execution')
):
  ...
  try:
    ...
    service = get_databricks_service()
    result = await service.get_job_cost_breakdown(job_id=job_id, run_id=run_id)

    if not result:
      raise HTTPException(
        status_code=404, detail=f'No cost breakdown found for job_id: {job_id}, run_id: {run_id}'
      )
    ...
    raise HTTPException(status_code=500, detail=f'Error retrieving job cost breakdown: {str(e)}')
...
async def get_top_jobs(
  ...
  try:
    ...
    service = get_databricks_service()
    result = await service.get_top_jobs(start_date=start_date, end_date=end_date, limit=limit)

    return result

  except Exception as e:
    raise HTTPException(status_code=500, detail=f'Error retrieving top jobs: {str(e)}')
...
async def get_date_presets():
  ...
  presets = {
    ...
    'this_month': {'label': 'This Month', 'start_date': today.replace(day=1), 'end_date': today},
    'last_7_days': {
      'label': 'Last 7 Days',
...
async def get_databricks_host():
  ...
  try:
    ...
    raise HTTPException(
      status_code=500, detail=f'Unable to determine Databricks workspace URL: {str(e)}'
    )
...
async def debug_environment():
  ...
  env_info = {
    ...
    'client_id_prefix': os.getenv('DATABRICKS_CLIENT_ID')[:10] + '...'
  ...
  return {'environment': env_info, 'client': client_info}
...
async def debug_table_data():
  ...
  try:
    ...
    sample_response = client.statement_execution.execute_statement(
      warehouse_id=service.warehouse_id, statement=f'SELECT * FROM {service.table_name} LIMIT 5'
    )
    ...
    if date_range_response.result and date_range_response.result.data_array:
      ...
      date_range = {'min_date': row[0], 'max_date': row[1]}
...
async def analyze_job_costs(
  ...
  try:
    ...
    breakdown, historical_stats, job_name = await asyncio.gather(
      ...
      service.get_job_historical_stats(job_id=job_id, current_run_id=run_id),
    ...
    if isinstance(breakdown, Exception):
      logger.error(
        'Failed to fetch cost breakdown for job %s run %s: %s', job_id, run_id, breakdown
      )
      breakdown = None
...
async def get_cluster_details(cluster_id: str):
  ...
  try:
    ...
    if not cluster_details:
      raise HTTPException(
        status_code=404, detail=f'Cluster details not found for cluster_id: {cluster_id}'
      )
    ...
    raise HTTPException(status_code=500, detail=f'Error retrieving cluster details: {str(e)}')
...
async def get_other_cost_breakdown(
  ...
  try:
    ...
    raise HTTPException(status_code=500, detail=f'Error retrieving other cost breakdown: {str(e)}')
...
async def get_classification_coverage_trend(
  ...
  try:
    ...
    raise HTTPException(status_code=500, detail=f'Error retrieving coverage trend: {str(e)}')
...
async def get_cloud_platform_config():
  ...
  try:
    ...
    return CloudPlatformInfo(
      ...
      enable_shared_clusters_tab=app_config.enable_shared_clusters_tab,
      enable_instance_pools_tab=app_config.enable_instance_pools_tab,
    )
  except Exception as e:
    raise HTTPException(
      status_code=500, detail=f'Error retrieving cloud platform configuration: {str(e)}'
    )
  ...
  try:
    ...
    try:
      response = client.statement_execution.execute_statement(
        warehouse_id=service.warehouse_id, statement='SELECT 1 as test_value'
      )
